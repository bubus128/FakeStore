...
public class CartService(IFakeStoreApiClient apiClient, ILogger<CartService> logger) : ICartService
{
    ...
    public async Task<Cart> GetCartAsync(int userId)
    {
        try
        {
            return await apiClient.GetCartAsyncByUserId(userId).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task AddToCartAsync(int userId, int productId)
    {
        try
        {
            var cart = await apiClient.GetCartAsyncByUserId(userId).ConfigureAwait(false);
            await apiClient.AddToCartAsync(cart.Id, productId).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    public async Task RemoveFromCartAsync(int userId, int productId)
    {
        ...
        {
            var cart = await apiClient.GetCartAsyncByUserId(userId).ConfigureAwait(false);
            await apiClient.RemoveFromCartAsync(cart.Id, productId).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
